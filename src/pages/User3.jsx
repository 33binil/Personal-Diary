    ...
    useEffect(() => {
        const load = async (showLoading = true) => {
            ...
            try {
                ...
                if (location.state?.action) {
                    setShowSavingPopup(true);
                    setTimeout(() => setShowSavingPopup(false), 3000);
                }
            }
        };
        load(true);
...
<PageIntroFade outOnly hold={500} fadeOut={300} />
                ...
                <div className="w-full max-w-[1750px] mt-6 flex flex-col gap-4">
                    {filteredEntriesByDate.length === 0 && (
                        <div className="text-center opacity-70">
                            {queryDate ? 'No entries for the selected date. Tap + to write a new entry.' : 'No entries yet. Tap + to write your first diary.'}
                        </div>
                    )}
                        ...
                        return (
                            <div key={date} className="bg-[#FCC37B] rounded-2xl p-5 md:p-7 px-6 md:px-24 w-full shadow-lg">
                                ...
                                <div className="mt-4 flex flex-col gap-3">
                                    {items.map((it) => (
                                        <div key={it.id} className="bg-black/10 rounded-xl p-3 md:p-4 relative cursor-pointer hover:bg-white/5" onClick={() => navigate(`/diary3?openId=${encodeURIComponent(it.id)}${queryDate ? `&date=${queryDate}` : ''}`)}>
                                            <button
            ...
            <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center justify-center gap-10 md:gap-20 xl:gap-44">
                {/* Left small button */}
                <button onClick={() => setShowCalendarOverlay(true)} className="w-10 h-10 md:w-[60px] md:h-[60px] rounded-full bg-[#C89C64] flex items-center justify-center
            ...
            {showCalendarOverlay && (
                <div className="fixed inset-0 z-[210] flex items-center justify-center bg-black/60 backdrop-blur-sm">
                    <div className="bg-[#C89C64]/10 backdrop-blur-lg rounded-2xl p-6 w-[92%] max-w-2xl border border-white/20 shadow-2xl">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="bg-white/5 backdrop-blur-sm p-4 rounded-xl border border-white/10">
                                ...
                                <input 
                                    type="date" 
                                    className="w-full p-3 rounded-lg bg-white/5 text-white border border-white/10 focus:outline-none focus:ring-2 focus:ring-[#C89C64] transition-all duration-200"
                                    onChange={(e) => {
                                        const v = e.target.value;
                                        if (!v) return;
                                        setShowCalendarOverlay(false);
                                        navigate(`/user3?date=${v}`);
                                    }}
                                />
                            </div>
                            <div className="bg-white/5 backdrop-blur-sm p-4 rounded-xl border border-white/10">
                                        ...
                                        <div className="space-y-2">
                                            {entriesByDate.map(ed => (
                                                ...
                                                >
                                                    ...
                                                    <span className="text-xs bg-[#C89C64]/20 text-[#C89C64] px-2 py-1 rounded-full">
